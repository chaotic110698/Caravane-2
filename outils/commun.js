...
const SECTIONS=['monde','personnages','objets','missions','evenements','icones'];
    ...
    return {
      ...
      entrees:liste.map(x=>({
        ...
        teinte:c.nom==='icones'?teinteDe(x)
              ...
              :teinteDe(x.ico||{}),
        lien:x.cle?c.atelier+'#cle='+encodeURIComponent(x.cle):c.atelier
      }))
    };
  });
}
...
function fichierDuJeu(section){
  ...
  if(!c||!D[section])return null;
  return {nom:FICHIERS_JEU[section],
          contenu:nettoyer(JSON.parse(JSON.stringify(D[section]))),
